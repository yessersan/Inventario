...
@section('title', 'Nuevo Cambio de Hardware')
@section('page-title', 'Registrar Cambio de Hardware')
...
<div class="row">
    <div class="col-lg-8 offset-lg-2">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('hardware-changes.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="change_type" id="change_type" class="form-select @error('change_type') is-invalid @enderror" required>
                                <option value="modificacion" {{ old('change_type') == 'modificacion' ? 'selected' : '' }}>Modificación</option>
                                <option value="reemplazo" {{ old('change_type') == 'reemplazo' ? 'selected' : '' }}>Reemplazo</option>
                                <option value="repotenciación" {{ old('change_type') == 'repotenciación' ? 'selected' : '' }}>Repotenciación</option>
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="responsible_id" id="responsible_id" class="form-select @error('responsible_id') is-invalid @enderror">
                                ...
                                @foreach($personnel as $p)
                                    <option value="{{ $p->id }}" {{ old('responsible_id') == $p->id ? 'selected' : '' }}>{{ $p->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="old_component_id" id="old_component_id" class="form-select @error('old_component_id') is-invalid @enderror">
                                ...
                                @foreach($components as $c)
                                    <option value="{{ $c->id }}" {{ old('old_component_id') == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->code }})</option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select name="new_component_id" id="new_component_id" class="form-select @error('new_component_id') is-invalid @enderror">
                                <option value="">Ninguno</option>
                                @foreach($components as $c)
                                    <option value="{{ $c->id }}" {{ old('new_component_id') == $c->id ? 'selected' : '' }}>{{ $c->name }} ({{ $c->code }})</option>
                                @endforeach
                            </select>
